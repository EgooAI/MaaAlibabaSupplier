...
应用日志和独立 Agent 日志分别写入 `backend/data/logs/api.log`、`agent.log`，采用 JSON 行格式。请求、后台队列、同步、outbox 和 Maa 原生任务通过关联字段连接；outbox 保存首次请求和账号代次，确认及重试保留各自的执行上下文。日志不是发送或送达凭证，记录缺失不能作为未执行的依据。
