    ...
    <Card title="账号接入" extra={<Button loading={refreshing} disabled={mutating} onClick={() => void refresh()}>刷新状态</Button>}>
      <Space orientation="vertical" size="middle" className="w-full">
        ...
        {!snapshot ? <Typography.Text>正在读取接入状态，请稍候。</Typography.Text> : <>
          ...
          <SyncStatus details buttonLabel="验证 Key 并同步" />
          <Space wrap>
          ...
          <Typography.Text type="secondary">接入仅初始化客户端，不执行点击。重新接入或窗口变化后需重新人工确认。首次验证与同步需你主动发起；Key 验证通过后，后端持续检查并同步新消息。数据迁移由后端自动备份后执行。</Typography.Text>
