  ...
  return [
    {
      key: "data_dir",
      title: "数据目录",
      description: dir.state === "ok" ? "目录有效" : dir.detail || "未配置数据目录",
      status: dir.state === "ok" ? "finish" : dir.state === "invalid" ? "error" : "wait",
    },
    {
      key: "identity",
      title: "卖家账号",
      description: account.self_ali_id ? `卖家 ${account.self_ali_id}` : "选择卖家账号",
    ...
    },
    {
      key: "crm",
      title: "聊天同步",
      description: source.ready ? "聊天已同步" : source.last_error || "待同步",
      status: source.ready ? "finish" : source.phase === "error" || source.last_error ? "error" : "wait",
    },
    {
      key: "client",
      title: "客户端确认",
      description: client.connected ? (client.confirmed ? "已确认卖家" : "待人工确认") : "未接入客户端",
      status: client.connected ? (client.confirmed ? "finish" : "process") : "wait",
    },
    {
      key: "model",
      title: "AI 模型",
      description: model.configured ? "模型已配置" : "未配置，前往 Agent → LLM",
