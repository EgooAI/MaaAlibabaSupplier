...
`shared/backend` 模块中提供了 `im_db_middleware.py`，暴露多种方法操作数据库，旨在：对外接口隐藏加密语义，对密钥进行懒加载（请求数据库信息时如果当前没有缓存的密钥再去调用解密器来进行密钥提取），对数据库进行带时效缓存的解密拉取（设置一个例如 5s 的缓存，请求数据库信息时如果缓存过期，就执行一次“解密 -> 复制解密后的数据库到程序缓存目录”的流程）。
