    ...
    with ui.card().classes("w-full p-4 gap-4"):
        with ui.row().classes("items-center justify-between w-full"):
            with ui.column().classes("gap-1"):
                ...
                ui.label("Chat 工具栏固定绑定的四个系统 Agent，可编辑并恢复默认配置。").classes("text-xs text-gray-500")
            refresh_btn = ui.button("刷新", icon="refresh").props("outline size=sm")
                ...
                for display_name, apid, helper_text in _SYSTEM_AGENT_DEFINITIONS:
                    ...
                    with ui.expansion(display_name, value=False).classes("w-full"):
                        with ui.card().classes("w-full rounded-2xl border border-slate-100 p-4 gap-4 shadow-sm"):
                            ...
                            def _make_restore_system_agent(preset_apid: str):
                                def _restore_agent() -> None:
                                    try:
                                        restore_system_agent_default(preset_apid)
                                    except Exception as exc:
                                        ui.notify(f"恢复默认设置失败：{exc}", type="negative")
                                        return
                                    ui.notify("已恢复默认设置", type="positive")
                                    _render()

                                return _restore_agent

                            if preset is None:
                                ui.label("该系统 Agent 在数据库中不存在，点击恢复默认设置将写入 SQL 默认值。").classes(
                                    "text-sm text-red-600"
                                )
                                ui.button(
                                    "恢复默认设置",
                                    icon="restart_alt",
                                    on_click=_make_restore_system_agent(apid),
                            ...
                            description = ui.input("描述", value=preset.description).props("required").classes("w-full")
                            prompt = _prompt_editor(value=preset.prompt)
                            ui.label("系统 Agent 不暴露工具；输出会在代码侧自动规范化。").classes(
                                "text-xs text-gray-500"
                            )
                            ...
                            with ui.row().classes("gap-2"):
                                ...
                                ui.button(
                                    "恢复默认设置",
                                    icon="restart_alt",
                                    on_click=_make_restore_system_agent(preset.apid),
                                ).props("outline color=warning size=sm")
